...
// Získanie order token z URL parametrov
...
try {
    ...
    // Získanie detailov objednávky podľa tokenu
    $sql = "SELECT 
                id, 
    ...
    $stmt->bindParam(':token', $orderToken, PDO::PARAM_STR);
    $stmt->execute();
    $order = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$order) {
        echo json_encode(['success' => false, 'message' => 'Objednávka s týmto kódom nebola nájdená']);
        exit;
    }
    ...
    $negotiationsStmt->bindParam(':order_id', $order['id'], PDO::PARAM_INT);
